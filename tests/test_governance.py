...
def test_gov01_ministry_upload(page: Page, request):
    ...
    location_indices = [int(i) for i in data_map.get("locationIndices").split("|")]
    ...
    try:
        ...
        governance_page.select_location_by_index(location_indices)
...
def test_gov04_role_personnel_mapping(page: Page, request):
    ...
    roles_file = data_map.get("RolesFile")
    officers_file = data_map.get("OfficersFile")
    dropdown_indices_str = data_map.get("DropdownSelections", "")

    
    dropdown_indices = [int(x) for x in dropdown_indices_str.split("|") if x.isdigit()]
    location_indices = [int(i) for i in data_map.get("locationIndices").split("|")]
    ...
    try:
        ...
        governance_page.fill_governance_dropdowns_by_index(dropdown_indices)

      
        governance_page.next_btn.click()
        governance_page.submit_btn.click()

      
        update_csv_and_report(page, request, "GOV04", expected, passed=True)

    except Exception as e:
       
        update_csv_and_report(page, request, "GOV04", expected, passed=False, error=str(e))
        raise
...
def test_gov06_update_roles(page: Page, request):
    ...
    dropdown_indices_str = data_map6.get("DropdownSelections", "")

    
    dropdown_indices = [int(x) for x in dropdown_indices_str.split("|") if x.isdigit()]
    location_indices = [int(i) for i in data_map6.get("locationIndices").split("|")]
